...
public class JSRun implements ServletCartridge {
	...
	/*public String fromPath(String path) {
		if (path.startsWith(LSPRE)) {
			path=path.substring(LSPRE.length());
	...
	public boolean get(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (u==null) u="/";
        if (u.startsWith("/"+JSRun.PATH_DIRECT_MODE)) {
        	direct(req,resp);
        	return true;
        }
        if (u.startsWith("/"+JSRun.PATH_EXEC)) {
        	exec(u.substring(JSRun.PATH_EXEC.length()+1), req,resp);
        	return true;
        }
		return false;
	}
